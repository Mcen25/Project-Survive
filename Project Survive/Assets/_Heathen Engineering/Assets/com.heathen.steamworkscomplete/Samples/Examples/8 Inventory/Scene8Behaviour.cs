...
namespace HeathenEngineering.DEMO
{
    ...
    public class Scene8Behaviour : MonoBehaviour
    {
        ...
        public void StartPurchase()
        {
            ...
            {
                Debug.LogWarning("You cannot fully test Start Purchase in the editor because it must open the Steam Overlay which will not work in editor.");
            }

            itemDefinition.StartPurchase((response, ioError) =>
            {
                if (!ioError)
                {
                    ...
                    {
                        Debug.LogError("Unexpected result from Valve: " + response.m_result);
                    }
                }
                else
                {
                    Debug.LogError("Valve indicated an IO Error occurred. i.e. failed to start the process at all.");
                }
            });
        }
        ...
        /// To understand more about item exchange please read the documentation.
        /// https://kb.heathenengineering.com/assets/steamworks/learning/core-concepts/inventory
        /// </summary>
        public void Exchange()
        {
            ...
            if (itemDefinition.CanExchange(itemDefinition.Recipes[0], out List<ExchangeEntry> recipe))
            {
                ...
                itemDefinition.Exchange(recipe, (response) =>
                {
                    ...
                    {
                        Debug.LogError("Unexpected result from Valve: " + response.result);
                    }
                });
            }
            else
